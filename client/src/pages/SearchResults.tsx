...
const SearchResults : React.FC = () => {
    ...
    const downloadCurrentPage = async(format: string) => {
      const imageUrl = `https://ilocr.iiit.ac.in${currentPageImage}`;
      const textUrl = `https://ilocr.iiit.ac.in${currentPageText}`;
      ...
      if (format === 'image') {
        ...
        for(let i = 0; i < bookPages.length; i++){
          ...
          image.src = `https://ilocr.iiit.ac.in/search${page.image}`;
    ...
    useEffect(() => {
      const fetchBookPages = async () => {
        try {
          const response = await axios.get<BookPage[]>(`https://ilocr.iiit.ac.in/search/api/book/${currentBook.id}/pages/`);
                ...
                <ModalContent>
                  ...
                  <ModalBody>
                    <div className="h-screen">
                      <Cropper ref={cropperRef} src={`https://ilocr.iiit.ac.in${currentPageImage}`} />
                    </div>
                  </ModalBody>
                  <ModalFooter>
